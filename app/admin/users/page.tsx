...
export default function UsersDirectory() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
                    ...
                    {filteredUsers.map((user) => (
                        <tr key={user.id} className={`hover:bg-gray-50 ${!user.approved ? 'bg-red-50' : ''}`}>
                            <td className="p-4 flex items-center gap-3">
                                {user.avatar_url ? <img src={user.avatar_url} className="w-10 h-10 rounded-full object-cover" /> : <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center">👤</div>}
                                <div><p className="font-bold text-gray-900">{user.full_name}</p><p className="text-xs text-gray-400">{user.role}</p></div>
                            </td>
                            <td className="p-4 text-sm"><p>{user.email}</p><p className="text-gray-500">{user.phone}</p></td>
                            <td className="p-4">
                                {user.approved ? <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">Activo</span> : <span className="text-xs bg-red-100 text-red-800 px-2 py-1 rounded">Pendiente</span>}
                            </td>
                            <td className="p-4 text-right flex justify-end gap-2">
                                {user.role !== 'admin' && <button onClick={() => toggleStatus(user.id, user.approved)} className={`text-xs font-bold px-3 py-2 rounded ${user.approved ? 'bg-gray-100 text-gray-500' : 'bg-green-600 text-white'}`}>{user.approved ? 'Bloquear' : 'Aprobar'}</button>}
                                <Link href={`/admin/users/${user.id}`} className="text-indigo-600 bg-indigo-50 px-3 py-2 rounded font-bold text-sm">✏️ Editar</Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
      ...
      {/* --- MODAL DE REGISTRO NUEVO (ACTUALIZADO) --- */}
      {isRegisterOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleCreateUser} className="p-6 space-y-4">
                    ...
                    {/* NUEVO: Fechas de Bautismo en dos columnas */}
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                    ...
                    <button type="submit" disabled={creating} className="w-full bg-green-600 text-white font-bold py-3 rounded hover:bg-green-700 disabled:opacity-50">
